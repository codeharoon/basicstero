...
<div class="content">
    <div class="mob-header-menu-icon">
        <img src="/static/img/mobnav.png" alt="">
    </div>
    ...
    <div class="header-in-item2">
                ...
                <a href="{{route('order')}}">
                    <div class="box-item4">My orders</div>
                </a>
                <a href="{{route('payment')}}">
                    <div class="box-item4">Payments</div>
                </a>
                <a href="{{route('referals')}}">
                    <div class="box-item4">Referals</div>
                </a>
                <a href="{{route('profile')}}">
                    <div class="box-item4">Profile</div>
                </a>
...
<script>
    ...
    $.ajaxSetup({headers: {'X-CSRF-TOKEN': '2TzQgAdUTAnSPKK1fd5rT9oBHhhLh4InMoukKhN3'}});
...
<script>
    ...
    <script>
        function calcPriceElemVisibility(total) {
            limit = 0;
            console.log(total);
            if (total >= limit || total <= 500) {
                $('.price_hider').hide('fast');
                $('.price_hider2').show('fast');
            } else {
                $('.price_hider').show('fast');
                $('.price_hider2').hide('fast');
            }
        }

        $(document).ready(function () {
            calcPriceElemVisibility(20);

            $('.pay_method_button').click(function () {
                setTimeout(function() {
        ...
        function setQuantity(product_id, quantity) {
            $.post('https://basicstero.ws/set_cart_quantity', {product_id: product_id, quantity: quantity}, function (response) {
                symbol = '$';
                data = JSON.parse(response);
                $('#delivery_price').html(data.delivery);
                $('#total_price').html(data.total);
                $('#cart_quantity').html(data.totalQuantity);
                $.each(data.productsConverted, function (id, price) {
                    $('#product_price_' + id).html(price);
                });
                calcPriceElemVisibility(data.totalInt);
                $('.warning').removeClass('warning');
                $.each(data.availableWarehouses, function (id, availableWarehouses) {
                    if (availableWarehouses.length == 0) $('#product_' + id).addClass('warning');
                });
                if (parseInt(data.selectedWarehouse) > 0) {
                    $('.order-header').show();
                    $('.preorder-header').hide();
                    $('.attention').stop().slideUp(300);
                } else {
                    $('.order-header').hide();
                    $('.preorder-header').show();
                    $('.attention').stop().slideDown(300);
                }
            });
        }
    </script>
